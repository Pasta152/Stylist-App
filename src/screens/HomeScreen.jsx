...
export default function HomeScreen({ weather, weatherLoading, ootd, ootdLoading, ootdLimitHit, wardrobe, onRemix, usageInfo }) {
  return (
    <motion.div
      key="home"
      initial={{ opacity: 0 }}
    ...
    >
      ...
      <div style={{ marginTop: 28 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {/* Usage counter */}
            {usageInfo && !ootdLimitHit && (
              <span style={{ fontSize: 10, color: 'var(--muted)', background: 'var(--s2)', padding: '3px 8px', borderRadius: 20 }}>
                {usageInfo.ootdRemaining} free left
              </span>
            )}

            <button
              onClick={onRemix}
              disabled={ootdLoading || ootdLimitHit}
              style={{
                display: 'flex', alignItems: 'center', gap: 6,
                background: 'var(--gold2)', border: '1px solid var(--gold3)',
                borderRadius: 20, padding: '6px 14px',
                cursor: (ootdLoading || ootdLimitHit) ? 'default' : 'pointer',
                opacity: (ootdLoading || ootdLimitHit) ? 0.5 : 1,
              }}
            >
